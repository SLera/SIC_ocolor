...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 16:24:05 2017
...
INDIR = '/home/valeria/nfs0/data_sonarc/data/heap/ice_concentration/raw/'
OUTDIR = '/home/valeria/nfs0/data_sonarc/data/heap/ice_concentration/processed_ocolor/'
...
def create8days_mean (filelist8):
    mean8_temp = read_ifremer_sic_daily(filelist8[0])
    name_mean8 = filelist8[0][-13:-5]
    for i in np.arange(1,len(filelist8)):
        mean8_temp  = mean8_temp +read_ifremer_sic_daily(filelist8[i])       
    mean8 = mean8_temp/len(filelist8)
...
def prepare_nsidc_ic_mean8 (mean8, output_tiff):
#    dataset = netCDF4.Dataset(input_nc)
##   ice_concentration = dataset.variables['concentration'][:][0]
#    sic = filter_array(dataset, filter1)
...
#def finddates_8mean(indate,filelist_nc):
#    return filelist8


#prepare_nsidc_ic_filtered ('19980101.nc','19980101_NaN_3.tif')

#mean8,name_mean8 =create8days_mean(flist8)
prepare_nsidc_ic_mean8 (mean8,'19980101_NaN_4.tif')

#for i in np.arange(len(years)):
#    filelist_year = glob.glob(INDIR+str(year[i])+'/*')
#filelist_year.sort()